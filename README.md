...
- **Urdu/English switch (started):** the profile's language now drives `MaterialApp.locale`, which flips the whole app right to left for Urdu automatically (Flutter ships Urdu translations for its own stock widgets) and adds a bundled Noto Nastaliq Urdu font as a fallback so any Urdu text anywhere renders properly (`lib/l10n/lang.dart`, wired in `lib/main.dart`). The Home screen and onboarding (first launch and Edit my profile, which now previews the chosen language live before saving) are fully translated both ways, along with the shared date/cycle-phase helpers Home uses; every other screen is still English-only and is translated next, screen by screen. Urdu wording has not been reviewed by a native speaker.
