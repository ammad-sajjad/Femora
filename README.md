...
- **Urdu/English switch (started):** the profile's language now drives `MaterialApp.locale`, which flips the whole app right to left for Urdu automatically (Flutter ships Urdu translations for its own stock widgets) and adds a bundled Noto Nastaliq Urdu font as a fallback so any Urdu text anywhere renders properly (`lib/l10n/lang.dart`, wired in `lib/main.dart`). The Home screen and the shared date/cycle-phase helpers are fully translated both ways as the first slice; every other screen is still English-only and is translated next, screen by screen. Urdu wording has not been reviewed by a native speaker.
