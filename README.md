...
Cycle tab: log the first and last day of each period (one tap, or any day on the calendar), see cycle day and phase, the expected next period with a likely window, the estimated ovulation day and fertile window, and notes about late or irregular cycles. Home shows the real cycle day; the AI companion and the health report use it too. Everything is stored on the phone, per account.

The prediction is the woman's own average cycle length blended with the study average (`lib/models/cycle_engine.dart`, constants in `lib/models/cycle_params.dart`, generated by `ml/cycle_eval.py`). It was chosen by measurement on the Fehring data (159 women, 1,665 cycles, tested only on unseen women): next period within 3 days about 67% of the time on day one and about 82% with 3+ cycles; a Random Forest did no better. Results: `ml/cycle_results.json`. To rerun: download `nikitabisht/menstrual-cycle-data` from Kaggle and run `ml/.venv/Scripts/python.exe ml/cycle_eval.py <path to the csv>`. Ovulation and the fertile window are estimates, not contraception.

## Tracking, insights, reminders and dashboard (scope 6.6 to 6.10)

- **Symptom and mood tracker (6.10):** the Cycle tab log now also takes sleep, stress and energy, for today, yesterday or any of the last 60 days. *See my trends* draws mood, sleep, stress, energy and symptoms over 7, 30 or 90 days with plain-language notes (`lib/models/trends.dart`, `lib/screens/trends_screen.dart`).
...
- **Report reader:** in the AI companion (card on an empty chat, or the settings menu) she photographs a lab test, hormone test, ultrasound report or prescription (up to 3 pages) and gets an explanation in Urdu or English with colour-coded values against the printed ranges and questions for her doctor (`POST /report/explain`, `backend/report_reader.py`, `lib/screens/report_reader_screen.dart`). Photos are cleaned (EXIF and location removed, shrunk) and read by Gemini through the server; nothing is stored on the server and the photo is never kept on the phone. It never diagnoses or gives doses, removes identifiers from the answer, and a critical value always raises the urgency. A consent dialog appears before the first photo. Needs a working `GEMINI_API_KEY`; not yet checked against the real service or a phone camera.
- **Urdu/English switch (started):** the profile's language now drives `MaterialApp.locale`, which flips the whole app right to left for Urdu automatically (Flutter ships Urdu translations for its own stock widgets) and adds a bundled Noto Nastaliq Urdu font as a fallback so any Urdu text anywhere renders properly (`lib/l10n/lang.dart`, wired in `lib/main.dart`). Every screen's own layout is now bilingual: Home, onboarding (with a live language preview before saving), the Cycle calendar and its daily log form, Dashboard, Trends, Hormonal insights, Reminders, the PCOS and breast-health questionnaires and result cards, the Self-Exam guide's navigation, the AI companion's menu and chat chrome, and the report reader's own screen. Deliberately left for a later pass: the generated clinical text (hormonal-insight flags/patterns, PCOS/breast guidance, reminder notification text, the Self-Exam Guide's instructions) and the whole PDF report, since these carry the most clinical weight and benefit most from a native speaker's review. Urdu wording overall has not been reviewed by a native speaker.
