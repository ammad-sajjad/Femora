...
class PlatformVoiceDevice implements VoiceDevice {
  ...
  Future<void> play(Uint8List audio) async {
    // The server sends MP3 from the neural voice, or WAV from the Gemini backup voice.
    final wav = audio.length > 4 && audio[0] == 0x52 && audio[1] == 0x49 && audio[2] == 0x46 && audio[3] == 0x46; // "RIFF"
    if (kIsWeb) {
      // A browser has no temporary folder; it plays the bytes directly.
      await _player.play(BytesSource(audio, mimeType: wav ? 'audio/wav' : 'audio/mpeg'));
      return;
    }
    // A file is more dependable than in-memory bytes on Android phones.
    final dir = await getTemporaryDirectory();
    final file = File('${dir.path}/femora_reply.${wav ? 'wav' : 'mp3'}');
...
class VoiceController extends ChangeNotifier {
  ...
  ///
  /// [natural] picks which voice is tried first. The natural voice comes from the server: Microsoft's neural
  /// voices, about 2 seconds for a short spoken answer (Gemini's voice, now only the server's backup, took 5
  /// to 16 seconds). The phone's own voice starts at once but sounds robotic, so it is the fallback.
  /// Failures are reported through [error] and never block the chat.
