...
table(["What", "How", "Result"], [
    ...
    ["Flutter tests", "Breast flows, PCOS flow, server-address logic, health store, chat state, companion screen with a fake microphone, onboarding and app launch, report builder", "329 of 329 pass; flutter analyze reports no errors and no warnings"],
...
table(["Suite", "Count", "What it covers"], [
    ...
    ["test/health_store_test.dart", "part of 319 new", "Saving and loading, one log entry a day and a 60-entry cap, name-free context text, clear-all"],
    ["test/chat_state_test.dart", "part of 319 new", "History rules, personalisation switch, retry of an unanswered message, persistence"],
    ["test/companion_flow_test.dart", "part of 319 new", "Typing indicator, suggestion chips, failed message and Try again, urgent highlight, full voice flow with a fake microphone, missing permission, speaker button, Delete all my data"],
    ["test/app_flow_test.dart", "part of 319 new", "First launch shows onboarding, skipping is remembered, form validation, editing a saved profile"],
    ["test/home_flow_test.dart", "part of 319 new", "Home shows the real name and only real results, empty state, rows open the right tab, next-step rules, relative dates"],
    ["test/auth_flow_test.dart (with test/fake_auth.dart)", "part of 319 new", "Sign-in, registration, a refused password, guest mode, the phone code, and two accounts not seeing each other's results"],
    ["test/cycle_engine_test.dart", "part of 319 new", "22 tests: no history, day one, personalising, missed logs, ovulation and fertile window, phases, unfinished period, late and irregular cycles, date maths"],
    ["test/period_store_test.dart", "part of 319 new", "11 tests: saving and reloading periods, ending a period, refused logs with reasons, separate accounts, deleting all data, companion context, older saved data"],
    ["test/cycle_screen_test.dart", "part of 319 new", "11 tests: first log, tracking display, month browsing, ending a period, refused log message, late and irregular notes, day menu, future days, symptom log"],
    ["test/trends_test.dart", "part of 319 new", "19 tests: log fields saved and loaded, mood scores, the day window, averages, symptom counts, each trend note and its threshold, the store (a year of logs, companion averages)"],
    ["test/log_form_test.dart", "part of 319 new", "7 tests: saving sleep, stress, energy, mood and symptoms; refilling from a saved day; clearing; logging yesterday; account switch; opening the trends"],
    ["test/trends_screen_test.dart", "part of 319 new", "5 tests: empty state, numbers and charts, 7 / 30 / 90 day ranges, charts with no data, too few days"],
    ["test/hormone_insights_test.dart", "part of 319 new", "25 tests: logs placed in phases, per-phase averages, each pattern and flag with its thresholds, too little data, unordered logs, phase background, hormone curve shapes, companion line"],
    ["test/hormone_screen_test.dart", "part of 319 new", "7 tests: empty state, table and patterns, too few logs, flagged symptom leading to the PCOS check, chart labels, opening from the Cycle tab, the new symptoms"],
    ["test/reminders_test.dart (with test/fake_reminders.dart)", "part of 319 new", "23 tests: planner rules, settings state with a recording stand-in for the phone, store hook"],
    ["test/reminders_screen_test.dart", "part of 319 new", "11 tests: switches, timing choices, permission refusal, unsupported platform, daily time, medication add / refuse / switch / delete, self-exam switch, opening from the Cycle tab"],
    ["test/analytics_test.dart", "part of 319 new", "11 tests: prediction replay on her own cycles, learning, irregular cycles, three-cycle minimum, duplicates and missed logs, cycle history figures"],
    ["test/dashboard_test.dart", "part of 319 new", "7 tests: empty states, headline numbers and charts, too few cycles, hormone sections, flagged symptom, latest results, links"],
    ["test/what_if_test.dart", "part of 319 new", "29 tests: what-if requests and answers, which changes are offered (no BMI under 18.5), live estimate with pauses and stale answers, errors, the card, the PCOS tab"],
    ["test/localization_test.dart", "part of 319 new", "7 tests: the pure direction/font helpers, an English profile stays left to right, an Urdu profile flips the whole app right to left including stock dialogs, switching from Edit my profile flips it live without losing the screen, the Urdu font fallback reaches the whole tree"],
    ["test/home_localization_test.dart", "part of 319 new", "15 tests: every Home card in Urdu, the next-step sentences in the same priority order as English, the relative-date and greeting helpers"],
    ["test/onboarding_localization_test.dart", "part of 319 new", "3 tests: the whole onboarding form in Urdu and right to left, the live language preview as English or اردو is tapped, Skip for now saving whichever language was previewed"],
    ["test/report_reader_test.dart", "part of 319 new", "38 tests: the report model and its companion line (no medicines, no name), the multipart request, the state, storage and accounts, consent, the screen in every state, Urdu, the speaker, the hand-off to the companion"],
    ["test/report_service_test.dart", "part of 319 new", "Empty, sample and full reports build as valid PDFs; long logs spill onto more pages; report IDs"],
], [5.6, 2.6, 8.4], caption="New tests", size=8.5,
    note="All 329 Flutter tests (10 earlier plus 319 new) and all 97 backend tests pass. The companion screen's real speech recognition and the real microphone are not covered by automatic tests because they need a phone.")
...
bullets([
    "**What the user sees.** Onboarding, and Edit my profile, already asked which language to use; what changed is that the rest of the app now honours the answer. Choosing Urdu turns the whole app right to left in one step (navigation order, alignment, stock dialogs, date pickers) because Urdu is one of the languages Flutter itself treats as right-to-left once the app's locale is set to it, and it now is. So far the Home screen (greeting, cycle card, health snapshot, quick log, next step, dashboard and trends cards) is fully translated both ways, along with the shared date formatting and cycle-phase names it uses. The onboarding screen (first launch and Edit my profile) is also fully translated, and previews the chosen language live as she taps English or اردو, before she saves it, so the language switch itself is visible immediately rather than only after the form is submitted. The rest of the app's screens still show their English text regardless of the language chosen, and are translated screen by screen next.",
    ...
    "**What is not done yet, stated plainly.** Every other screen (cycle calendar, PCOS and breast screens, reminders, trends, hormonal insights, the AI companion's own screen text, the report reader, the PDF report) is still English-only; the companion's own replies already answer in Urdu when asked, independently of this switch. The Urdu wording written so far, including onboarding, has not been checked by a native Urdu speaker.",
])
