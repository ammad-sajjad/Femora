...
table(["What", "How", "Result"], [
    ...
    ["Flutter tests", "Breast flows, PCOS flow, server-address logic, health store, chat state, companion screen with a fake microphone, onboarding and app launch, report builder", "350 of 350 pass; flutter analyze reports no errors and no warnings"],
...
table(["Suite", "Count", "What it covers"], [
    ...
    ["test/health_store_test.dart", "part of 340 new", "Saving and loading, one log entry a day and a 60-entry cap, name-free context text, clear-all"],
    ["test/chat_state_test.dart", "part of 340 new", "History rules, personalisation switch, retry of an unanswered message, persistence"],
    ["test/companion_flow_test.dart", "part of 340 new", "Typing indicator, suggestion chips, failed message and Try again, urgent highlight, full voice flow with a fake microphone, missing permission, speaker button, Delete all my data"],
    ["test/app_flow_test.dart", "part of 340 new", "First launch shows onboarding, skipping is remembered, form validation, editing a saved profile"],
    ["test/home_flow_test.dart", "part of 340 new", "Home shows the real name and only real results, empty state, rows open the right tab, next-step rules, relative dates"],
    ["test/auth_flow_test.dart (with test/fake_auth.dart)", "part of 340 new", "Sign-in, registration, a refused password, guest mode, the phone code, and two accounts not seeing each other's results"],
    ["test/cycle_engine_test.dart", "part of 340 new", "22 tests: no history, day one, personalising, missed logs, ovulation and fertile window, phases, unfinished period, late and irregular cycles, date maths"],
    ["test/period_store_test.dart", "part of 340 new", "11 tests: saving and reloading periods, ending a period, refused logs with reasons, separate accounts, deleting all data, companion context, older saved data"],
    ["test/cycle_screen_test.dart", "part of 340 new", "11 tests: first log, tracking display, month browsing, ending a period, refused log message, late and irregular notes, day menu, future days, symptom log"],
    ["test/trends_test.dart", "part of 340 new", "19 tests: log fields saved and loaded, mood scores, the day window, averages, symptom counts, each trend note and its threshold, the store (a year of logs, companion averages)"],
    ["test/log_form_test.dart", "part of 340 new", "7 tests: saving sleep, stress, energy, mood and symptoms; refilling from a saved day; clearing; logging yesterday; account switch; opening the trends"],
    ["test/trends_screen_test.dart", "part of 340 new", "5 tests: empty state, numbers and charts, 7 / 30 / 90 day ranges, charts with no data, too few days"],
    ["test/hormone_insights_test.dart", "part of 340 new", "25 tests: logs placed in phases, per-phase averages, each pattern and flag with its thresholds, too little data, unordered logs, phase background, hormone curve shapes, companion line"],
    ["test/hormone_screen_test.dart", "part of 340 new", "7 tests: empty state, table and patterns, too few logs, flagged symptom leading to the PCOS check, chart labels, opening from the Cycle tab, the new symptoms"],
    ["test/reminders_test.dart (with test/fake_reminders.dart)", "part of 340 new", "23 tests: planner rules, settings state with a recording stand-in for the phone, store hook"],
    ["test/reminders_screen_test.dart", "part of 340 new", "11 tests: switches, timing choices, permission refusal, unsupported platform, daily time, medication add / refuse / switch / delete, self-exam switch, opening from the Cycle tab"],
    ["test/analytics_test.dart", "part of 340 new", "11 tests: prediction replay on her own cycles, learning, irregular cycles, three-cycle minimum, duplicates and missed logs, cycle history figures"],
    ["test/dashboard_test.dart", "part of 340 new", "7 tests: empty states, headline numbers and charts, too few cycles, hormone sections, flagged symptom, latest results, links"],
    ["test/what_if_test.dart", "part of 340 new", "29 tests: what-if requests and answers, which changes are offered (no BMI under 18.5), live estimate with pauses and stale answers, errors, the card, the PCOS tab"],
    ["test/localization_test.dart", "part of 340 new", "7 tests: the pure direction/font helpers, an English profile stays left to right, an Urdu profile flips the whole app right to left including stock dialogs, switching from Edit my profile flips it live without losing the screen, the Urdu font fallback reaches the whole tree"],
    ["test/home_localization_test.dart", "part of 340 new", "15 tests: every Home card in Urdu, the next-step sentences in the same priority order as English, the relative-date and greeting helpers"],
    ["test/cycle_screen_localization_test.dart", "part of 340 new", "4 tests: the Cycle screen's title and buttons, the whole daily log form (day chooser, header, symptoms, mood, sleep, notes, save) in Urdu, the Another-day date and save label, the cycle-phase tip"],
    ["test/reminders_screen_localization_test.dart", "part of 340 new", "4 tests: titles and switches, the period reminder options and daily time button, the medication dialog, an Urdu day/month/time in the upcoming list"],
    ["test/pcos_localization_test.dart", "part of 340 new", "4 tests: the start card, the whole questionnaire, the missing-choices error, the result card and its buttons"],
    ["test/breast_localization_test.dart", "part of 340 new", "5 tests: the upload and risk-start cards, the how-is-this-calculated dialog, the self-exam card, the risk result card, the questionnaire opened from Start Questionnaire"],
    ["test/companion_localization_test.dart", "part of 340 new", "4 tests: the empty state and disclaimer, the settings menu and delete-everything dialog, an urgent reply's warning label, a failed message's retry button"],
    ["test/onboarding_localization_test.dart", "part of 340 new", "3 tests: the whole onboarding form in Urdu and right to left, the live language preview as English or اردو is tapped, Skip for now saving whichever language was previewed"],
    ["test/report_reader_test.dart", "part of 340 new", "38 tests: the report model and its companion line (no medicines, no name), the multipart request, the state, storage and accounts, consent, the screen in every state, Urdu, the speaker, the hand-off to the companion"],
    ["test/report_service_test.dart", "part of 340 new", "Empty, sample and full reports build as valid PDFs; long logs spill onto more pages; report IDs"],
], [5.6, 2.6, 8.4], caption="New tests", size=8.5,
    note="All 350 Flutter tests (10 earlier plus 340 new) and all 97 backend tests pass. The companion screen's real speech recognition and the real microphone are not covered by automatic tests because they need a phone.")
...
H2("16.3 Urdu and English language switch")
bullets([
    "**What the user sees.** Onboarding, and Edit my profile, already asked which language to use; what changed is that the rest of the app now honours the answer. Choosing Urdu turns the whole app right to left in one step (navigation order, alignment, stock dialogs, date pickers) because Urdu is one of the languages Flutter itself treats as right-to-left once the app's locale is set to it, and it now is. So far the Home screen (greeting, cycle card, health snapshot, quick log, next step, dashboard and trends cards) is fully translated both ways, along with the shared date formatting and cycle-phase names it uses. The onboarding screen (first launch and Edit my profile) is fully translated, and previews the chosen language live as she taps English or اردو, before she saves it. Every other screen's own layout (navigation, titles, buttons, forms, empty states, dialogs, snackbars) is now bilingual too: the Cycle calendar and its daily log form, the Dashboard, Trends and Hormonal insights screens, Reminders (including the medication dialog), the PCOS and breast-health questionnaires and their result cards, the Self-Exam guide's navigation, and the AI companion's settings menu, empty state, message bubbles and errors. The report reader (feature A) picked up the remaining pieces of its own screen: the consent dialog, its buttons and a report's kind and value status are now shown in the report's own language.",
    "**How it works.** MaterialApp now takes its `locale` from the signed-in profile and lists Flutter's own English and Urdu localisations, so the automatic right-to-left layout and the built-in translations of stock widgets come for free. A bundled font (Noto Nastaliq Urdu, SIL Open Font Licence) is added as a fallback to the app's default text style, so any Urdu character drawn anywhere in the app is shown in a proper Nastaliq face instead of whatever the phone would otherwise substitute, without editing the hundreds of existing Text widgets one by one. A translated screen picks its English or Urdu sentence with a small `t(language, english, urdu)` helper (`tList` for a list of options) next to the string it replaces, which keeps the two languages next to each other for review. A few values (a symptom's name, a cycle phase, a report's kind or status, a relative date) are stored or matched in English internally but shown with a matching `xIn(language)` accessor, so nothing that other code reads or writes changes shape.",
    "**Tests.** 7 tests on the switching mechanism itself (locale and direction flip for an Urdu profile, English is unaffected, switching from Edit my profile flips the running app without losing its state, the font fallback reaches the whole tree), 15 on the Home screen in Urdu (every card, the priority-ordered next-step sentences, and the relative-date and greeting helpers), and 3 on onboarding (the whole form in Urdu and right to left, the live preview as the language is tapped and reverted, Skip for now saving whichever language was previewed).",
    "**What is deliberately left for later, stated plainly.** The generated clinical explanations — hormonal-insight flags, patterns and phase tips beyond the phase name itself, PCOS and breast-cancer guidance bullets and factor labels, the reminder text that actually appears in a phone notification, the Self-Exam Guide's own step-by-step instructions, and the whole PDF report — are still English-only. These carry the most clinical weight and the most benefit from a native speaker's review, so they were left rather than translated quickly. The sign-in screen is also English-only, since the language choice itself lives in the profile, which does not exist until after signing in. The companion's own replies already answer in Urdu when asked, independently of this switch. All Urdu wording written so far has not been checked by a native Urdu speaker.",
])
